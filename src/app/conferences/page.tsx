...
export default function ConferenceListPage() {
  ...
  useEffect(() => {
    ...
    const result = conferences.filter((item) => {
      const matchesSearch = search
        ? item.title.toLowerCase().includes(search) ||
          item.scholarship_name.toLowerCase().includes(search)
        : true
    ...
    setFiltered(result)
  }, [debouncedSearch, debouncedCountry, funding, conferences])
    ...
    <main className="max-w-7xl mx-auto px-4 py-10 space-y-10">
      ...
      <section className="text-center space-y-3">
        ...
        <p className="text-muted-foreground max-w-2xl mx-auto text-base md:text-lg">
          Connect with global leaders and experts. Find funded opportunities to
          attend major conferences worldwide.
        </p>
      </section>
      ...
      <section className="flex flex-wrap gap-4 items-start">
        <Input
          aria-label="Search conference or scholarship"
          placeholder="🔍 Search conference or scholarship"
          value={searchInput}
