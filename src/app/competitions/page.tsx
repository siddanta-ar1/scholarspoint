...
type Competition = {
  id: string
  title: string
  organization: string | null // Can be null
  deadline: string | null // Can be null
  country: string | null // Can be null
  location: string | null // Can be null
  funding_type: FundingType | null // Can be null
...
export default function CompetitionListPage() {
  ...
  const [filtered, setFiltered] = useState<Competition[]>([])
  ...
  const uniqueCountries = useMemo(() => {
    ...
    competitions.forEach((c) => {
      if (c.country) set.add(c.country.trim())
    })
  ...
  useEffect(() => {
    ...
    setFiltered(result)
  }, [debouncedSearch, debouncedCountry, funding, competitions])
    ...
    <main className="max-w-7xl mx-auto px-4 py-10 space-y-10">
      ...
      <section className="text-center space-y-3">
        ...
        <p className="text-muted-foreground max-w-2xl mx-auto text-base md:text-lg">
          Showcase your skills and win amazing prizes by participating in global
          competitions.
        </p>
      </section>
      ...
      <section>
        {loading ? (
          ...
          <p className="text-center text-gray-500 text-lg">
            No competitions found for the selected filters.
          </p>
        ) : (
              ...
              >
                <Card className="h-full flex flex-col max-w-[400px] mx-auto group rounded-2xl overflow-hidden border border-gray-200 dark:border-neutral-700 shadow-md hover:shadow-xl hover:border-sky-300 transition duration-300 ease-in-out transform hover:scale-[1.02] bg-white dark:bg-neutral-900 cursor-pointer">
                  ...
                  <CardContent className="flex flex-col flex-grow p-5 space-y-2">
                    ...
                    {/* --- FIX 2: Safely handle funding_type --- */}
