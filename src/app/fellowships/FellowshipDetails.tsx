...
export default function FellowshipDetail() {
  ...
  useEffect(() => {
    const fetchFellowship = async () => {
      if (!id) return;
      const { data, error } = await supabase.from('fellowships').select('*').eq('id', id).single()
      if (error) {
  ...
  const jsonLd = fellowship ? {
    "@context": "https://schema.org",
    "@type": "Scholarship",
    "name": fellowship.title,
    "description": fellowship.description,
    "image": fellowship.image_url,
    "funder": {
      "@type": "Organization",
      "name": fellowship.organization
    },
    "url": window.location.href,
    "validThrough": fellowship.deadline,
    "occupationalCategory": fellowship.field_of_study,
    "financialAidEligible": "Full Funding",
    "amount": {
      "@type": "MonetaryAmount",
      "currency": "USD",
      "value": fellowship.amount
    }
  } : null;
  ...
  if (isExpired) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-10 text-center bg-background min-h-screen">
        ...
        <p className="text-gray-600">Please check out our other opportunities.</p>
        <Button asChild className="mt-6 bg-primary hover:bg-primary/90 text-primary-foreground">
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-10 space-y-10 bg-background text-gray-800">
        <nav className="flex items-center justify-between mb-4">
          <Link href="/fellowships" className="text-sky-600 font-bold flex items-center gap-2 hover:opacity-80">
            <ArrowLeft size={20} /> Back
          </Link>
        
        </nav>
      {/* Injecting Structured Data for SEO Sitelinks/Rich Results */}
      {jsonLd && (
        <script
        ...
        {fellowship.countries && fellowship.countries.length > 0 && (
          <section className="p-6 bg-card rounded-lg shadow-sm border border-border">
            ...
            <p className="text-gray-700">
              ...
              A few examples include: <span className="font-medium">{fellowship.countries.slice(0, 5).join(', ')}</span>.
            </p>
          </section>
        )}
